...
# ----------------------------------------
# If fp32Add is one of several IP cores in your
...
# ----------------------------------------
# ACDS 25.1.1 125 linux 2026.06.01.09:23:21
...
if ![info exists TOP_LEVEL_NAME] { 
  set TOP_LEVEL_NAME "fp32Add.fp32Add"
}
...
set LD_LIBRARY_PATH [dict merge $LD_LIBRARY_PATH [dict get [fp32Add::get_env_variables $SIMULATOR_TOOL_BITNESS] "LD_LIBRARY_PATH"]]
if {[dict size $LD_LIBRARY_PATH] !=0 } {
  set LD_LIBRARY_PATH [subst [join [dict keys $LD_LIBRARY_PATH] ":"]]
  setenv LD_LIBRARY_PATH "$LD_LIBRARY_PATH"
}
append ELAB_OPTIONS [subst [fp32Add::get_elab_options $SIMULATOR_TOOL_BITNESS]]
append SIM_OPTIONS [subst [fp32Add::get_sim_options $SIMULATOR_TOOL_BITNESS]]
...
alias file_copy {
  ...
  set memory_files [concat $memory_files [fp32Add::get_memory_files "$QSYS_SIMDIR" "$QUARTUS_INSTALL_DIR"]]
  ...
  set normalized_src [fp32Add::normalize_path "$file"]
  set normalized_dest [fp32Add::normalize_path "$dest_file"]
...
set libraries [dict merge $libraries [fp32Add::get_dpi_libraries "$QSYS_SIMDIR"]]
...
set design_libraries [dict merge $design_libraries [fp32Add::get_design_libraries]]
...
alias com {
  ...
  set common_design_files [dict values [fp32Add::get_common_design_files "$QSYS_SIMDIR"]]
  
  set design_files [list]
  set design_files [concat $design_files [fp32Add::get_design_files "$QSYS_SIMDIR" "$QUARTUS_INSTALL_DIR"]]
