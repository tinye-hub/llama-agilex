# Full compile + simulate LlamaM2aTop M2a graduation test.
...
set DEFAULT_M2A_DDR [file join $REPO_ROOT tools ddr_pack out ddr_image.bin]
...
if {$DO_WAVE} {
    ...
    if {[info exists env(QUESTA_WLF)] && $env(QUESTA_WLF) ne ""} {
        ...
        set WLF_FILE [file join $WORK_DIR tb_llama_m2a_top.wlf]
    }
    append elab_cmd " -wlf \"$WLF_FILE\""
}
append elab_cmd " tb_llama_m2a_top"
