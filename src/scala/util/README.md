...
Documented as `latency` constants in `IntelFloatIPCollection.scala`. Native DSP values should be confirmed in Questa before closing timing on the full design.

RMSNorm 模块目录：`rmsNorm/`（`scala/` 源码、`test/questa/` 仿真、`gen/` 生成物、`quartus/` 工程）。推荐：
...
Requires SpinalHDL **1.12.2+** (see `build.sbt`) and Questa on `PATH` (`source activate.sh`).
