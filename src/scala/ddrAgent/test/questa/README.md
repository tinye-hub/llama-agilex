...
make questa-m1       # wraps: $NC_RUN ./run.sh m1
```

Direct (local Questa license):

```bash
make questa-m1 NC_RUN=
# or: cd test/questa && ./run.sh m1
```

```bash
make questa-m1-wave              # records test/questa/work/tb_ddr_agent_m1.wlf
make questa-m1 WAVE=1            # equivalent
...
- Default AXI data width is **256-bit** (`make verilog`, `make questa-m1`).
- Verilator `make sim` still uses **64-bit** AXI internally (WData compat).
