# Repository paths for RoPE Questa simulation.

if {![info exists env(ROPE_QUESTA_DIR)] || $env(ROPE_QUESTA_DIR) eq ""} {
    echo "ERROR: ROPE_QUESTA_DIR not set — run via ./run.sh"
    exit 1
}
set QUESTA_DIR [file normalize $env(ROPE_QUESTA_DIR)]
set ROPE_TEST_DIR [file dirname $QUESTA_DIR]
set ROPE_DIR [file dirname $ROPE_TEST_DIR]
...
set GEN_TOP_V [file join $ROPE_DIR gen verilog SerialRoPEAxiTop.v]
...
if {![file exists $GEN_TOP_V]} {
    echo "ERROR: missing $GEN_TOP_V — run: make verilog (from src/scala/rope)"
    exit 1
}
...
set ::env(ROPE_QUESTA_WORK) $WORK_DIR
